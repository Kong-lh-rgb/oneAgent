...
当前 Runtime 在每次模型请求前生成具名的临时 `oneagent_runtime_environment` system
message，包含本地 ISO 日期时间、UTC offset 和时区名称。它与 Memory、Task Context、
Checkpoint 一样属于请求视图，不进入 AgentResult 和 SQLite 原始历史。工具执行后再次
请求模型时也重新生成，因此跨日的长 Run 不依赖第一次请求时间。

旧会话仍保存过去写入的固定日期。迁移时不直接修改数据库，而是在模型请求副本中识别
`当前日期是 YYYY-MM-DD。` 并移除，再注入新的动态时间。这样模型不会同时收到冲突日期，
