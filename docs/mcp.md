...
当前只支持 `stdio`。默认配置路径是 `backend/.oneagent/mcp.json`，也可以通过 CLI
...
`${EXAMPLE_API_KEY}` 会从 OneAgent 进程环境读取；变量不存在时只将该 Server 标记为
...
其他 Server 或 OneAgent 主流程启动。退出 CLI 时会关闭 MCP Session 和 stdio 子进程。
